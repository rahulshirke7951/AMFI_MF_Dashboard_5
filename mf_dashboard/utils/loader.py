...
RETURN_COLS = {
    "return_7d":    "1W",  "return_14d":   "2W",
    "return_30d":   "1M",  "return_90d":   "3M",
    "return_180d":  "6M",  "return_365d":  "1Y",
    "return_730d":  "2Y",  "return_1095d": "3Y",
    "cagr_3y":      "3Y CAGR",
}
...
def _split_category(cat_str) -> tuple:
    ...
    level1 = m.group(1).strip()
    inner  = m.group(2).strip()

    parts  = [p.strip() for p in inner.split(" - ", 1)]
    level2 = parts[0] if len(parts) > 0 else "NA"
    level3 = parts[1] if len(parts) > 1 else "NA"
...
# ─────────────────────────────────────────────
# EXCEL LOADER (LOCKED ARCHITECTURE)
# ─────────────────────────────────────────────
def load_excel(file) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # 🧹 Remove Level 1 permanently
    df = df.drop(columns=["cat_level_1"], errors="ignore")
